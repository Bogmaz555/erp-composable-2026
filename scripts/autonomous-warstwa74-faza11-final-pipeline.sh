#!/usr/bin/env bash
# W74 FINAL — Faza 11 Frontend BI + CI Auth Workflow + BI Projections (W71–W73) + Faza 10 aggregate
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT" && source scripts/erp-env.sh
LOG="${ROOT}/.agents/swarm/warstwa74-faza11-final.log"
mkdir -p "$(dirname "$LOG")"
log() { echo "[$(date -Iseconds)] $*" | tee -a "$LOG"; }

log "W74 FAZA11 FINAL START"
...
nohup bash -c "${PNPM} --filter analytics-service run start:dev" >> /tmp/erp-w74-4011.log 2>&1 &
nohup bash -c "${PNPM} --filter plm-service run start:dev" >> /tmp/erp-w74-plm.log 2>&1 &
sleep 20
...
for i in $(seq 1 25); do
  a=$(curl -s -o /dev/null -w '%{http_code}' http://127.0.0.1:4011/platform/frontend-bi/readiness 2>/dev/null || echo 0)
...
npx tsx scripts/frontend-bi-readiness-smoke.ts 2>&1 | tee -a "$LOG" || true
npx tsx scripts/ci-auth-readiness-smoke.ts 2>&1 | tee -a "$LOG" || true
npx tsx scripts/bi-projection-readiness-smoke.ts 2>&1 | tee -a "$LOG" || true
npx tsx scripts/faza11-frontend-bi-final-smoke.ts 2>&1 | tee -a "$LOG" || true
npx tsx scripts/master-regression-report.ts 2>&1 | tail -10 | tee -a "$LOG" || true
log "W74 FAZA11 FINAL END"
