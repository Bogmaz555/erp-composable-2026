#!/usr/bin/env node
/**
 * Advance STATUS phase machine: DESIGN→IMPLEMENT→GATE→RELEASE→(next DESIGN|DONE)
 * Usage: node advance-phase.mjs [success|fail] [message]
 */
...
let milestone = getField(block, 'milestone') || 'Q0';
if (phase === 'BOOTSTRAP') phase = 'DESIGN';
...
if (result === 'fail') {
  ...
  if (phase === 'RELEASE' || idx === order.length - 1) {
    ...
    if (mi >= 0 && mi < ids.length - 1) {
      ...
      block = setField(block, `checklist`, getField(block, 'checklist')); // keep
    } else {
      milestone = 'DONE';
      phase = 'DONE';
...
block = setField(
  ...
  phase === 'DONE'
    ? 'none'
    : `Execute docs/enterprise-2.0/NEXT_AGENT_PROMPT.md (${milestone}/${phase})`,
);
block = setField(
  block,
  'resume_prompt',
  ...
  milestone=${milestone} phase=${phase}. Continue autonomy. No confirmation pauses.`,
);
