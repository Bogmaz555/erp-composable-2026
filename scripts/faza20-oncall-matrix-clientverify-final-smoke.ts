...
async function run() {
  console.log('=== Faza 20 Oncall Matrix ClientVerify FINAL (W110) ===\n');
  const paths = [
    '/api/analytics/platform/alert-oncall/readiness',
    '/api/analytics/platform/playwright-matrix/readiness',
    '/api/analytics/platform/mtls-client-verify/readiness',
    '/api/analytics/platform/mtls-proxy/readiness',
  ];
  let fails = 0;
