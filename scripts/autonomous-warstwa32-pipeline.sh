#!/usr/bin/env bash
# W32 — TD-010 NestJS overrides ujednolicenie
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT" && source scripts/erp-env.sh
LOG="${ROOT}/.agents/swarm/warstwa32-pipeline.log"
mkdir -p "$(dirname "$LOG")"
log() { echo "[$(date -Iseconds)] $*" | tee -a "$LOG"; }

service_health() {
  curl -s -o /dev/null -w "%{http_code}" --max-time 3 "http://127.0.0.1:$1${2:-/health}" 2>/dev/null || echo "000"
}

restart_if_down() {
  local port=$1 cmd=$2
  if [[ ! "$(service_health "$port")" =~ ^[23] ]]; then
    log "restart :${port}"
    fuser -k "${port}/tcp" 2>/dev/null || true
    sleep 1
    nohup bash -c "$cmd" >> "/tmp/erp-w32-${port}.log" 2>&1 &
    sleep 14
  fi
}

log "W32 START"
npx tsx scripts/nestjs-version-audit.ts 2>&1 | tail -6 | tee -a "$LOG" || true
bash scripts/nest-build-all.sh 2>&1 | tail -4 | tee -a "$LOG" || true
restart_if_down 4011 "${PNPM} --filter analytics-service run start:dev"
restart_if_down 4005 "${PNPM} --filter api-gateway run start:dev" /api/health
pnpm run test:contracts 2>&1 | tail -4 | tee -a "$LOG" || true
npx tsx scripts/nestjs-version-smoke.ts 2>&1 | tee -a "$LOG" || true
npx tsx scripts/master-regression-report.ts 2>&1 | tail -8 | tee -a "$LOG" || true
log "W32 END"
