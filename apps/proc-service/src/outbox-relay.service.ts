...
@Injectable()
export class ProcOutboxRelayService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(ProcOutboxRelayService.name);
  ...
  async relayEvents() {
    ...
    for (const event of pending) {
      try {
        ...
        this.logger.debug(`[PROC Outbox] Relayed ${event.eventType}`);
      } catch (e) {
        ...
        await this.prisma.outboxEvent
          ...
          .catch(() => {});
        this.logger.warn(
          `[PROC Outbox] ${dead ? 'DEAD-LETTER' : 'retry'} ${event.eventType} (attempt ${attempts}/${this.maxAttempts}): ${(e as Error).message}`,
        );
      }
    }
  }
