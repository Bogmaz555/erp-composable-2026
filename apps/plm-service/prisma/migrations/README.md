...
```bash
PILOT=1 bash scripts/prisma-migrate-deploy.sh plm-service
```
...
npx prisma@5.22.0 migrate diff \
  --from-empty \
  --to-schema-datamodel apps/plm-service/prisma/schema.prisma \
  --script > apps/plm-service/prisma/migrations/20260801000000_baseline/migration.sql
```
