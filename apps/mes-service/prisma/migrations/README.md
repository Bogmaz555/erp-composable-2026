...
```bash
PILOT=1 bash scripts/prisma-migrate-deploy.sh mes-service
```
...
npx prisma@5.22.0 migrate resolve --applied 20260801000000_baseline \
  --schema apps/mes-service/prisma/schema.prisma
...
npx prisma@5.22.0 migrate diff \
  --from-empty \
  --to-schema-datamodel apps/mes-service/prisma/schema.prisma \
  --script > apps/mes-service/prisma/migrations/20260801000000_baseline/migration.sql
```
