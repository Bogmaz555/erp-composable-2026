...
export function usePMProjects() {
  return useQuery<Project[]>({
    ...
    queryFn: async () => {
      const res = await fetchWithAuth('/api/pm');
...
export function useUpdateWBSElement() {
  ...
  return useMutation({
    mutationFn: async (payload: {
      ...
      const res = await fetchWithAuth('/api/pm/wbs', {
        method: 'PATCH',
...
export function useProjectTasks(projectId: string) {
  return useQuery<Task[]>({
    ...
    queryFn: async () => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/tasks`);
...
export function useCreateTask() {
  ...
  return useMutation({
    mutationFn: async (payload: { projectId: string; title: string }) => {
      const res = await fetchWithAuth(`/api/pm/projects/${payload.projectId}/tasks`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useRequestMaterial() {
  ...
  return useMutation({
    mutationFn: async (payload: { projectId: string; taskId: string; sku: string; quantity: number }) => {
      const res = await fetchWithAuth(`/api/pm/projects/${payload.projectId}/tasks/${payload.taskId}/materials`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function useReleaseProject() {
  ...
  return useMutation({
    mutationFn: async (projectId: string) => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/release`, {
        method: 'POST',
      });
...
export function usePMSchedule(projectId: string) {
  return useQuery<ProjectSchedule>({
    ...
    queryFn: async () => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/schedule`);
...
export function usePMEvm(projectId: string) {
  return useQuery<ProjectEvm>({
    ...
    queryFn: async () => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/evm`);
...
export function usePMBaselineCompare(projectId: string) {
  return useQuery<BaselineCompare>({
    ...
    queryFn: async () => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/baseline/compare`);
...
export function useCreatePMBaseline() {
  ...
  return useMutation({
    mutationFn: async (payload: { projectId: string; name?: string }) => {
      const res = await fetchWithAuth(`/api/pm/projects/${payload.projectId}/baseline`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export function usePMResourceLevel() {
  return useMutation({
    mutationFn: async (projectId: string) => {
      const res = await fetchWithAuth(`/api/pm/projects/${projectId}/resources/level`, { method: 'POST' });
