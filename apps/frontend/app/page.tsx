...
export default function PremiumDashboard() {
  const [activeTab, setActiveTab] = useState('Wskaźniki Finansowe')
  ...
  const metrics = [
    { id: 1, label: 'Status Bramki KSeF', val: 'Online', icon: ShieldCheck, color: 'text-green-400' },
    { id: 2, label: 'Wiadomości NATS/s', val: mps.toString(), icon: Zap, color: 'text-yellow-400' },
    { id: 3, label: 'Suma Zdarzeń', val: totalEvents.toLocaleString(), icon: Gauge, color: 'text-purple-400' },
    { id: 4, label: 'Aktywne Mikroserwisy', val: `${activeServices}/11`, icon: Cpu, color: 'text-cyan-400' },
  ];
  ...
  return (
    <div className="flex h-screen w-full relative z-10 p-4 gap-6 text-foreground">
      ...
      <main className="flex-1 flex flex-col gap-6 overflow-hidden pr-2 z-20 animate-fade-in" style={{ animationDelay: '100ms' }}>
        ...
        <header className="glass-panel h-20 flex items-center justify-between px-8">
          <div>
            <h2 className="text-xl font-semibold text-white tracking-wide">
              Obszar Roboczy: {activeTab}
            </h2>
            <p className="text-sm text-gray-400">Next.js 15 Native + Działająca Sieć Zero Trust</p>
          </div>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-4 py-2 bg-green-500/10 border border-green-500/20 rounded-full">
              ...
              <span className="text-xs font-semibold text-green-400 uppercase tracking-wider">Wszystkie Systemy Działają</span>
            </div>
        ...
        {/* HORIZONTAL TABS */}
        <div className="flex items-center gap-2 px-8 z-20">
          {[
            { icon: LayoutDashboard, label: 'Wskaźniki Finansowe' },
            { icon: Factory, label: 'Łańcuch Dostaw i Produkcja' },
            { icon: Boxes, label: 'Gotowość Operacyjna' },
            { icon: Activity, label: 'Infrastruktura IT' },
            { icon: Zap, label: 'Command Center AI' },
          ].map((item) => (
            <button
              key={item.label}
              onClick={() => setActiveTab(item.label)}
              className={`flex items-center gap-2 px-5 py-2.5 rounded-full transition-all duration-300 text-sm font-semibold shadow-sm border
                ${activeTab === item.label 
                  ? 'bg-gradient-to-r from-indigo-600 to-purple-600 text-white border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.4)]' 
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-8 pb-8 flex flex-col gap-6">
          {activeTab === 'Wskaźniki Finansowe' && <CeoDashboard />}
          {activeTab === 'Command Center AI' && <CommandCenterAI />}

        {/* --- LIVE TELEMETRY TAB --- */}
        {activeTab === 'Infrastruktura IT' && (
          <>
            <CommandCenterPanel />
            ...
            <div className="flex-1 glass-panel glass-hover p-6 flex flex-col relative overflow-hidden animate-slide-up" style={{ animationDelay: '400ms' }}>
               ...
               <div className="flex items-center justify-between mb-8 z-10">
                 <div>
                   <h3 className="text-lg font-semibold text-white">Telemetria Event Sourcing</h3>
                   <p className="text-sm text-gray-400">Przepływ komend i zdarzeń w czasie rzeczywistym z NATS JetStream</p>
                 </div>
               ...
               {/* Mockup Chart Area */}
               <div className="flex-1 min-h-[300px] rounded-xl border border-white/5 bg-black/20 flex items-end justify-between p-8 gap-2 z-10 relative overflow-hidden">
        ...
        {/* --- APS GANTT TAB --- */}
        {activeTab === 'Łańcuch Dostaw i Produkcja' && (
          <div className="flex flex-col gap-6 animate-fade-in">
