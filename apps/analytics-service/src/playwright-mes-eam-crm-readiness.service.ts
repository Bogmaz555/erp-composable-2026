...
@Injectable()
export class PlaywrightMesEamCrmReadinessService {
  private findRepoRoot() {
    let dir = process.cwd();
    for (let i = 0; i < 8; i++) {
      if (fs.existsSync(path.join(dir, 'scripts/ci-playwright-mes-eam-crm-probe.ts'))) return dir;
  ...
  async getReadiness() {
    ...
    const chainSpec = fs.existsSync(path.join(root, 'e2e/mes-eam-crm-chain.spec.ts'));
    const probeScript = fs.existsSync(path.join(root, 'scripts/ci-playwright-mes-eam-crm-probe.ts'));
    ...
    try {
      ...
      workflowChainJob = workflow.includes('playwright-mes-eam-crm-chain');
      const block =
    ...
    try {
      ...
      ciGateIncludesProbe = gate.includes('ci-playwright-mes-eam-crm-probe');
    } catch {
    ...
    return {
      ...
      ciGateIncludesProbe,
      ciPlaywrightMesEamCrmEnv: process.env.CI_PLAYWRIGHT_MES_EAM_CRM === 'true',
      capabilities: ['MES → EAM → CRM cross-module E2E chain'],
