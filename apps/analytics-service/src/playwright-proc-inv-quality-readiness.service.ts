...
@Injectable()
export class PlaywrightProcInvQualityReadinessService {
  private findRepoRoot() {
    let dir = process.cwd();
    for (let i = 0; i < 8; i++) {
      if (fs.existsSync(path.join(dir, 'scripts/ci-playwright-proc-inv-quality-probe.ts'))) return dir;
  ...
  async getReadiness() {
    ...
    const chainSpec = fs.existsSync(path.join(root, 'e2e/proc-inv-quality-chain.spec.ts'));
    const probeScript = fs.existsSync(path.join(root, 'scripts/ci-playwright-proc-inv-quality-probe.ts'));
    ...
    try {
      ...
      workflowChainJob = workflow.includes('playwright-proc-inv-quality-chain');
      const block =
    ...
    try {
      ...
      ciGateIncludesProbe = gate.includes('ci-playwright-proc-inv-quality-probe');
    } catch {
    ...
    return {
      ...
      domain: 'PLAYWRIGHT_PROC_INV_QUALITY',
      chainSpec,
      probeScript,
      bootScript,
      workflowChainJob,
      ...
      ciGateIncludesProbe,
      ciPlaywrightProcInvQualityEnv: process.env.CI_PLAYWRIGHT_PROC_INV_QUALITY === 'true',
      capabilities: ['PROC → INV → Quality cross-module E2E chain'],
