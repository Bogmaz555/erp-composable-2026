...
```bash
PILOT=1 bash scripts/prisma-migrate-deploy.sh inv-service
```
...
npx prisma@5.22.0 migrate resolve --applied 20260801000000_baseline \
  --schema apps/inv-service/prisma/schema.prisma
...
npx prisma@5.22.0 migrate diff \
  --from-empty \
  --to-schema-datamodel apps/inv-service/prisma/schema.prisma \
  --script > apps/inv-service/prisma/migrations/20260801000000_baseline/migration.sql
```
